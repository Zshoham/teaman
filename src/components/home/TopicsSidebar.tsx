import { useState, type ReactNode } from "react";
import { ChevronDownIcon } from "lucide-react";

import { cn } from "@/lib/utils";

interface Topic {
  tag: string;
  count: number;
}

interface Props {
  topics: Topic[];
  /** How many topics to show before the "show more" toggle. */
  initialShown?: number;
  /** Section heading. Defaults to "topics". */
  title?: string;
  /** Optional content rendered above the topics list (e.g. a result line). */
  header?: ReactNode;
  /**
   * Controlled mode: the set of active tags. When `onToggle` is also given the
   * component owns its active state in React (multi-select); otherwise it stays
   * uncontrolled and the DOM `list-controller` drives `.is-active` via `data-tag`.
   */
  activeTags?: Set<string>;
  /** Controlled mode: called with the clicked tag. */
  onToggle?: (tag: string) => void;
}

export function TopicsSidebar({
  topics,
  initialShown = 8,
  title = "topics",
  header,
  activeTags,
  onToggle,
}: Props) {
  const [expanded, setExpanded] = useState(false);
  const overflow = Math.max(0, topics.length - initialShown);
  const controlled = typeof onToggle === "function";

  return (
    <aside className="flex flex-col gap-9 self-start md:sticky md:top-[calc(var(--header-h)_+_1.5rem)]">
      {header && <div>{header}</div>}
      <section>
        <h2 className="m-0 mb-3 border-b border-border pb-2 font-mono text-meta-sm font-normal uppercase tracking-label text-faint">
          {title}
        </h2>
        <div className="flex flex-col gap-0.5" data-topics>
          {topics.map((t, i) => {
            const active = controlled ? Boolean(activeTags?.has(t.tag)) : undefined;
            return (
              <button
                key={t.tag}
                type="button"
                className={cn(
                  "topic flex cursor-pointer items-baseline justify-between border-0 border-b border-dashed border-border bg-transparent py-1 text-left font-mono text-meta-lg text-muted-foreground transition-colors hover:text-foreground [&.is-active]:text-foreground",
                  active && "is-active",
                )}
                data-tag={t.tag}
                data-topic-index={i}
                aria-pressed={controlled ? (active ? "true" : "false") : undefined}
                onClick={controlled ? () => onToggle!(t.tag) : undefined}
                hidden={!expanded && i >= initialShown}
              >
                <span>
                  <span className="text-faint [.topic.is-active_&]:text-primary">#</span>
                  {t.tag}
                </span>
                <span className="tabular-nums text-faint">{String(t.count).padStart(2, "0")}</span>
              </button>
            );
          })}
        </div>
        {overflow > 0 && (
          <button
            type="button"
            className="mt-2.5 inline-flex cursor-pointer items-center gap-1.5 border-0 bg-transparent py-1 font-mono text-meta text-primary transition-colors"
            onClick={() => setExpanded((e) => !e)}
            aria-expanded={expanded}
          >
            <span>{expanded ? "show fewer" : `show ${overflow} more`}</span>
            <ChevronDownIcon
              className={cn("size-3 text-faint transition-transform", expanded && "rotate-180")}
              aria-hidden="true"
            />
          </button>
        )}
      </section>
    </aside>
  );
}
