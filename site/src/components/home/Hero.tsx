...
export function Hero({ eyebrow, title, description, statRows, lastUpdated }: Props) {
  return (
    <section className="grid items-end gap-8 border-b border-border py-9 md:grid-cols-[minmax(0,1fr)_280px] md:gap-16 md:pt-9 md:pb-11">
      <div>
        <p className="m-0 font-mono text-[11px] uppercase tracking-[0.08em] text-faint">{eyebrow}</p>
        <h1
          className="mt-3.5 font-serif text-4xl leading-[1.05] font-normal tracking-tight text-balance text-foreground md:text-[56px] [&>em]:italic [&>em]:text-muted-foreground"
      ...
      <aside className="flex flex-col gap-1.5">
        ...
        <div className="flex items-baseline justify-between gap-3 pt-2.5 font-mono">
          <span className="text-[11px] text-faint">last edit</span>
          <span className="tabular-nums text-[11px] text-muted-foreground">{relTime(lastUpdated)}</span>
        </div>
      </aside>
    </section>
  );
