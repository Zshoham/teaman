...
export default {
  ...
  slides: {                     // project-wide Slidev styling (all optional)
    logo: 'assets/logo.svg',    // shown at the bottom-right of every slide
    primary: 'oklch(0.62 0.15 48)',
    secondary: 'oklch(0.58 0.05 196)',
    footer: true,               // show the footer logo (default true)
  },
};
```
...
- `logo` — a mark shown in the bottom-right corner of every slide (resolved like
  `logo`: vault `public/`, then the vault root). Omit for none.
- `primary` / `secondary` — accent colors (any CSS color). Defaults match the
  site's warm accent.
- `footer` — show the footer logo (default `true`); set `false` to hide it.
